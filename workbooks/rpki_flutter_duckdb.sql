...
CREATE TEMP TABLE rpki_history AS SELECT * FROM read_parquet("rpki-flutter-boundaries.parquet");
...
CREATE TEMP TABLE rpki_by_allocation AS (
    SELECT 
        type, vp, gen_ts, capture_ts, asn, pfx, maxlen, rir, country, afi, length, date, status, opaque_id, category, resource
