...
from typing import Any, List, Union, Generator, TextIO, Set, Optional
...
def fields_from_file(csvfile: str) -> List[str]:
    ...
    if header != VERSION_MARKER:
        LOG.error("Version line '%s' in %s", header, file_name)
...
def read_file(
        ...
        filter_func: Optional[Any] = None) -> pd.DataFrame:

    with gzip.open(file_name, 'rt') if file_name.endswith('.gz') else open(file_name, 'r') as csvfile:
        ...
        try:
            ...
            if 'cs(User-Agent)' in this_df:
                this_df['cs(User-Agent)'] = this_df['cs(User-Agent)'].map(lambda x:urllib.parse.unquote(x))
...
def read_from_glob(
        ...
        final_func: Optional[Any] = None) -> pd.DataFrame:
    """
...
def main(aggregate_fields: List[str], globs: List[str]):
    ...
    if not aggregate_fields:
        print(f"Provide fields to group by via --fields")
